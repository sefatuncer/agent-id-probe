...
|---|---|---|---|
| 2026-08-17 | **R9.5** | **A sensitivity pair was added after collection**: `identifier_comparison_sensitivity` reports C12 and C13 under three readings of *identical* — byte for byte, the published RFC 3986 equivalences, and the case-insensitivity RFC 3986 makes mandatory without the port and empty-path equivalences. `tests/test_identifier_sensitivity.py` fires each transformation separately | R9.5 fixes the pairs before collection, so this is a departure and is entered as one in the manuscript's deviation table. It was added because U4 records that the clauses ask for code-point identity while the instrument normalises first, and nothing said what that was worth. It is worth **2.8** points on the resource identifiers and **2.4** on the issuers; the third reading returns the strict figures exactly, so no pair in this corpus differs by case alone and the whole of the gap is default ports and the empty path. **The ordering the results rest on is unchanged under all three**, which is the outcome that made the objection closable rather than the reason for running it |
| 2026-08-17 | **R10.1** | **Failure counts are additionally reported as unique deployments**: endpoints, apexes and implementation clusters carrying at least one MUST-level failure, with the co-occurrence distribution | A count of verdicts is not a count of deployments and reads as one. The census produced **1,506** verdicts over **1,324** endpoints, **858** registrable domains and **517** implementation clusters, with 86.3% of affected endpoints failing exactly one check and none failing all three. A plausible reading, that a few shared library defaults reproduce most of it, is **not** supported: the ten largest clusters carry 28.8%. The statistic makes the study's largest number smaller and is declared as a post-collection addition |
| 2026-08-14 | **R6 / R10.7** | **C05's primary denominator is the endpoints that answered an authorization challenge.** `rate_by_unit(challenged_only=True)` reports it at all three units; the wider population stays as a declared sensitivity arm, and `analysis.denominator_composition` publishes the split by first-response status. `tests/test_challenged_denominator.py` fails if the two denominators ever coincide | §6.1 stated that C05's denominator was *not* widened to "challenged or publishes metadata", on the ground that the widening would be circular for a check whose numerator is publishing metadata. The instrument did exactly that: R10.7 moved the metadata request ahead of the posture inference, so an endpoint that never challenged but published fell through to scoring, and **1,375 of C05's 2,976 endpoints, 46.2%, had not challenged**. The circularity is the one the section names — the only route in without a challenge is by satisfying the numerator — and it runs in the direction that flatters the study. **C05 moves 82.5% → 77.8% at the endpoint unit, 85.5% → 81.2% at the apex and 73.0% → 65.3% at the implementation**, and the corpus share that challenged is 18.6%, not the 33.5% §6.1 presented as endpoints that "opted into authorization at all". Found by a referee who ran the instrument rather than reading it, and confirmed independently before being accepted |
| 2026-08-14 | **Replication comparison** | The Zhou et al. bracket's lower end is the endpoints that **challenged**, and the upper end adds every method-gated endpoint. The two are disjoint by status | The published bracket used C05's denominator as its lower end and then added all method-gated endpoints on top, **counting 684 of them twice**. Corrected it reads **18.6%–47.5% over all endpoints** and **24.7%–63.3% over reachable endpoints**, against **33.5%–62.4%** and **44.5%–83.1%** before. The correction is **against this paper**: Zhou's 59.45% now falls outside the bracket over all endpoints and inside it only over reachable ones, so the corroboration is weaker than reported and depends on which denominator is read |
| 2026-08-14 | **R11.2 / §6.5** | A contrast between two populations is **tested** before it is stated. `analysis.fisher_exact_2x2` is applied to crossing against non-crossing issuers for both C18 and C16 | §6.5 called the split "the sharpest way to state the result" and the paper had a bound on the *level* of an observed zero where the sentence needed a test of a *contrast*. Two referees objected independently. The exact tests give **p = 0.62** for C18 and **p = 0.53** for C16: with 12 publishers over 1,981 issuers, a subset of 202 holding none of them is close to what independence predicts. The counts are unchanged and the level statements stand; the contrast is withdrawn |
| 2026-08-14 | **R10.4** | The design-effect ceiling test carries a **numerical** tolerance only (`RELATIVE_EPSILON`, 1e-9), not the 10% it carried before. The flagged count over the published intervals moves **2 → 3**, adding C05 at the endpoint unit | §4.5 states the criterion as a bright line and reports the count of intervals failing it as machine-derived, so an undocumented constant was setting that count. It was setting it here: C05's endpoint row sits at **10.71 against a ceiling of 10.62** and went unflagged for that reason alone while **one cluster carries 57.0%** of its between-cluster sum of squares. A zero-tolerance rule was tried first and rejected on the evidence: it marked eight rows, five of them collapsed-unit rows where DEFF and the ceiling are the same quantity and agree to fifteen digits, with two otherwise identical rows falling on opposite sides of the last bit |
| 2026-08-14 | **R10.4** | The boundary substitution's divisor is **floored at 1**, as the variance already was | A design effect below one meant the substitution evaluated Wilson at *more* observations than were collected, manufacturing precision no sample of this size could have — the exact thing the variance floor twenty lines above exists to forbid. Search over random cluster configurations reaches the combination in roughly one case in twenty-six, so the branch is not hypothetical, though **no published figure was affected**. Recorded here rather than in the manuscript because it changes no result |
| 2026-08-14 | **R10.4** | The coverage simulation measures the **published** estimators, not only the naive one, over three added boundary-adjacent scenarios matching the corpus's own 9-of-202 shape. The quoted naive range moves **20%–88% → 20%–89%** | The range licensing the whole interval methodology was measured for an estimator this study does not publish. Measured, the cluster-robust interval covers **69%–97%** and Wilson at the effective sample size **73%–95%**, so the substitute is **not worse than the estimator it replaces** and neither reaches nominal in the hardest shape. The naive range moves only because the new scenarios belong in it: quoting it over the seven original shapes would have kept §4.5's sentence intact and would have been a range chosen over a subset of the scenarios actually run, which is also the mismatch a reader re-running the script would find |
| 2026-08-13 | **R10.4** | **A published interval may not be clamped onto a boundary the data excludes.** Where the symmetric t interval would print 0 or 1 while the count is neither, the published interval becomes Wilson evaluated at the effective sample size — boundary-respecting, and still carrying the design effect rather than dropping it to rescue the bound. `tests/test_cluster_diagnostics.py` fails if either the clamp returns or the substitution fires on a genuine zero | Two referees read `4.5% [0.0%, 9.0%]` and objected that nine successes in 202 observations cannot be consistent with a rate of exactly zero. They were right and the estimator was the cause: the interval is symmetric around the point estimate and clamped into [0, 1] afterwards, so the clamp published a bound the observations exclude. **Exactly one figure in the manuscript changes**, C16 over the crossing issuers, from `[0.0%, 9.0%]` to `[1.7%, 11.4%]`. It is **wider and less favourable to the paper's own emphasis on scarcity**, and is therefore declared in the manuscript's deviation table as well as here. A machine-derived count of the intervals the rule touches is printed in §4.5, so the claim "this changes one figure" cannot go stale |
| 2026-08-13 | **R10.4** | **`n_eff` is no longer published as an effective sample size where the design effect exceeds what the cluster sizes can produce.** `ProportionEstimate` now carries Kish's effective cluster size, the ICC = 1 ceiling `kish · m/(m−1)`, the share of the between-cluster sum of squares held by the largest cluster, and the cluster-size profile. Two of the seventeen published intervals are flagged; Table 5 marks the affected cell and §§4.5, 6.2 and 6.4 name the cluster carrying it | A referee asked how an effective sample of **123** can sit below **1,814** clusters. Both obvious answers are wrong. It is not an arithmetic fault — a ratio estimator weights clusters by size, so `n_eff < m` is a real loss and C16's own rate shows it harmlessly. Nor is it mere skew: the measured design effect of **20.1** is above the algebraic ceiling of **9.9** that the cluster sizes impose, which is only possible when the outcome depends on cluster size. It does: **one apex makes 80 crossing declarations, all naming a single issuer, and carries 87.8% of the between-cluster sum of squares** (C13's endpoint row is the same shape, 91.3% from one apex). **The interval is correct and unchanged** — one operator really can move this rate, and an interval that hid it would be the defective one. What was wrong was the gloss: "123 effective observations" invites a reading about independence that the number does not carry. Only the gloss is withdrawn |
| 2026-08-10 | **R4 / R10.4** | **`OUT_OF_SCOPE` endpoints leave the narrowest unobserved population** used for §9.2's Manski bound, and join the row that already collects what the access policy declined to ask. The kill-switch share stays on its own definition, because the threshold it is quoted against counts those endpoints. `analysis.unreachable_composition` now reports what the unobserved fraction is made of, keyed to `ErrorKind` rather than to free text, and `tests/test_unreachable_composition.py` fails if a bucket empties silently | **A fourth instance of the same inversion, found by an external review reading the section as written.** §9.2 named WAFs as the mechanism behind the endpoints that did not answer and gave no number, so a reviewer concluded the study systematically loses hardened enterprise deployments. The corpus does not support that: of 1,156 unobserved endpoints, **37 are access blocks** — 0.4% of the corpus, and identically 37 in both census runs — while **658 fail at DNS, TLS or the connection**, meaning the host is gone. 222 time out and are assigned to neither side, since a bot manager's silent drop and an abandoned host are indistinguishable from outside. Separately, **238 were in that row because of our own per-host ceiling or scope rule**, which `fetcher.ErrorKind` already declares in the instrument's source, before collection, to be "our decision, so it leaves every denominator" — this was the denominator it did not leave. Effect: row 1 narrows, `b` 13.0% → 10.3%, C16 [5.4%, 18.4%] → [5.6%, 15.9%] and C18 [0.5%, 13.5%] → [0.5%, 10.9%]. **The narrowing favours us and is therefore declared in the manuscript's deviation table as well as here.** Rows 2 and 3 are unchanged to the endpoint: the population moved between rows, not out of the table |
| 2026-08-07 | **R6 / R10.1** | **Two populations leave every denominator that were leaving only one of them.** (a) `UNSPECIFIED` now leaves, stated in R6's body above and reported as its own column in the exclusion ledger. (b) Endpoints the access policy declined to ask — `robots.txt`, opt-out, cross-origin redirect — now leave `analysis.rate_by_unit`, which is what computes the published per-check rates; the funnel already removed them | The manuscript prints the funnel and the three-unit table on facing pages and they were computed under different rules, so they disagreed: **C05 read 2,455/2,975 in one and 2,471/2,996 in the other**, C12 by 196 endpoints and C13 by 717. Two external reviews read the disagreement as the study contradicting itself, which is the correct reading of it. Neither half was defensible on its own terms. Retaining a robots exclusion publishes **our politeness policy as a property of a third-party deployment** — the identical inversion as the kill-switch row of 30 July and the R11.5/D8 row, now found for a third time in a third module. Retaining `UNSPECIFIED` scores **our own uncertainty against the operator**, which R6 exists to forbid, and it silently made the published C12 rate the *strict* half of a pair Section 6.3 introduces as the lenient one. **Nothing tested either rule**: `tests/test_denominator_agreement.py` now fixes both in one place and fails if either computation moves without the other. Effect: C05 is now identical in both tables; C12 rises 70.5% → 71.4% at the funnel; **the headline does not move** (C16, 6.2%), and rank 4 moves 69.5% → 71.0% |
...
**An `UNSPECIFIED` observation leaves every denominator** (added 2026-08-07; see the log). It
is not a pass and not a failure, so a rate that retains it reports the instrument's own
uncertainty as the deployment's failure — which is the single thing this rule exists to
prevent. Where the treatment of a class is itself contestable, the cost of the other reading
is published as a pre-declared sensitivity pair rather than argued.
...
**20%–89%**, depending on the scenario — at `m = 300` clusters with a shared SDK default it is
