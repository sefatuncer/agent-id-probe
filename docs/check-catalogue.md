...
| C07 | `WWW_AUTH_RESOURCE_METADATA` | - | `should` | no | MCP Authorization, discovery | https://modelcontextprotocol.io/specification/latest/basic/authorization | `checks_oauth.py` x1 |
| C08 | `SENDER_CONSTRAINED` | - | `may` | yes | RFC 9449 DPoP and mTLS sender-constraining are optional in MCP | https://modelcontextprotocol.io/specification/latest/basic/authorization | `checks_oauth.py` x1 |
...
| C11 | `TLS_VALID` | - | `must` | no | MCP: all authorization server endpoints MUST be served over HTTPS; BCP 195<br>MCP: endpoints MUST be served over HTTPS | https://modelcontextprotocol.io/specification/latest/basic/authorization | `checks_oauth.py` x2 |
| C12 | `PRM_RESOURCE_IDENTITY_MATCH` | `oauth_metadata` | `must` | no | MCP Authorization<br>RFC 9728 3.3<br>RFC 9728 3.3: the resource value returned MUST be identical | https://modelcontextprotocol.io/specification/latest/basic/authorization<br>https://www.rfc-editor.org/rfc/rfc9728.html | `checks_oauth.py` x5 |
| C13 | `AS_CORRESPONDENCE` | `oauth_metadata` | `must` | no | MCP Authorization<br>RFC 8414 3.3: issuer value MUST be identical to the issuer requested<br>RFC 8414 3<br>RFC 8414 3.3 | https://modelcontextprotocol.io/specification/latest/basic/authorization<br>https://www.rfc-editor.org/rfc/rfc9728.html<br>https://www.rfc-editor.org/rfc/rfc8414.html | `checks_oauth.py` x10 |
| C14 | `PKCE_DECLARED` | `oauth_metadata` | `must` | no | MCP Authorization<br>MCP: absent code_challenge_methods_supported means clients MUST refuse | https://modelcontextprotocol.io/specification/latest/basic/authorization<br>https://www.rfc-editor.org/rfc/rfc9728.html | `checks_oauth.py` x6 |
...
| C17 | `CLIENT_BOOTSTRAP_DECLARED` | - | `should` | yes | MCP: clients fall back to prompting the user when no registration mechanism is advertised | https://modelcontextprotocol.io/specification/latest/basic/authorization | `checks_oauth.py` x1 |
...
- 67 emission sites across 2 modules.
